...
# Uses lazy init or safe check so imports don't crash if env vars are missing during test runs
supabase_base: Optional[Client] = None
...
def get_system_db_for_audit(operation_name: str, authorized_actor_id: str):
    ...
    if not supabase_service:
        raise RuntimeError(
            "Service role key not configured. System operation blocked."
        )
